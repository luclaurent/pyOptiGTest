...
class FunBiggsExp5(TestFunction):
    def evaluate(self, X, grad=False):
        ...
        p = np.zeros_like(xxx)
        for it in range(nbt):
            p = p + (zzz * np.exp(-t[it] * xxx) - lll * np.exp(-t[it] * yyy) + c * np.exp(-t[it] * mmm) - y[it])**2

        if grad:
            dp = np.zeros((xxx.shape[0], 5))
            for it in range(nbt):
                mult = zzz * np.exp(-t[it] * xxx) - lll * np.exp(-t[it] * yyy) + c * np.exp(-t[it] * mmm) - y[it]
                ...
                dp[:, 4] += -2 * c * t[it] * np.exp(-t[it] * mmm) * mult

        return (p, dp) if grad else p



funBiggsExp5 = FunBiggsExp5()
