...
class FunExp6(TestFunction):
    def evaluate(self, X, grad=False):
        ...
        pc = -np.exp(-listI / a)
        pd = b * np.exp(-listI)
        pe = np.exp(-listI[np.newaxis, :] * w[:, np.newaxis] / a)
        pf = -d * np.exp(-listI * f / a)

        pt = z[:, np.newaxis] * pa + v[:, np.newaxis] * pb + pc[np.newaxis, :] + pd[np.newaxis, :] + u[:, np.newaxis] * pe + pf[np.newaxis, :]
        ...
        if grad:
            ...
            dp[:, 4] = 2 * u * np.sum(-listI[np.newaxis, :] / a * pe * pt, axis=1)
            dp[:, 5] = 2 * np.sum(pe * pt, axis=1)
