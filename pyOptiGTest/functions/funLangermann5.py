...
class FunLangermann5(TestFunction):
    def evaluate(self, X, grad=False):
        ...
        ma = np.array([
            ...
            [4.138,2.562,2.532,9.661,5.611,5.500,6.886,2.341,9.699,6.500]]).T

        mc = np.array([0.806,0.517,0.100,0.908,0.965,0.669,0.524,0.902,0.531,
        ...
        for it in range(m):
            vx = X - ma[:dim, it][np.newaxis, :]
            svx = np.sum(vx ** 2, axis=1)
            ex = np.exp(-1.0 / d_pi * svx)
            cx = np.cos(d_pi * svx)
            cc = cx * mc[it]
            p = p - ex * cc
        ...
        if grad:
            ...
            for it in range(m):
                ...
                sx = np.sin(d_pi * svx)
                cs = sx * mc[it]
                pvx = vx * ex[:, np.newaxis]
                for itD in range(dim):
                    dp[:, itD] = dp[:, itD] + (2.0 / d_pi * cc + 2.0 * d_pi * cs) * pvx[:, itD]

        return (p, dp) if grad else p


funLangermann5 = FunLangermann5()
